    ...
    extends StochasticAgent
{
    ...
    public static final double GAMMA = 0.8; // feel free to change this around!
    ...
    public void valueIteration(StateView state)
    {
        ...
        do {
            delta = 0.0;
            Map<Coordinate, Double> uPrime = new HashMap<>();
    
            for (Coordinate c : u.keySet()) {
                if (isTerminalState(c)) {
                    uPrime.put(c, RewardFunction.getReward(c));
                    continue;
                }
                ...
                for (Direction d : TransitionModel.CARDINAL_DIRECTIONS) {
                    ...
                    for (Pair<Coordinate, Double> transition : TransitionModel.getTransitionProbs(state, c, d)) {
                        ...
                        actionValue += prob * u.get(nextState);
                    }
    
                    actionValue *= GAMMA; // multiply after summing
                ...
                double updatedValue = RewardFunction.getReward(c) + maxActionValue;
    
                delta = Math.max(delta, Math.abs(u.get(c) - updatedValue));
                uPrime.put(c, updatedValue);
            }
